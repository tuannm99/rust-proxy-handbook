...
(`06-proxy/retry.md`), which added *more* load. You are now spending your
...
point (`07-security/ddos.md`'s cost asymmetry). So shedding belongs early
in the pipeline (`09-architecture/components.md`): before auth, before
...
where you can, and sample the logging (`08-observability/logging.md`)
...
design decision:
- **By priority.** Health checks (`06-proxy/healthcheck.md`) and critical
...
- **By cost.** Expensive routes shed sooner, so one costly endpoint
  ...
  (`07-security/ddos.md`'s expensive-endpoint section). Per-route
...
misses at 200ms — `05-http-stack/cache.md`). Apply it per route, or per
...
(`08-observability/metrics.md`). They have completely different causes and
...
budget (`08-observability/alerting.md`).

...
(`06-proxy/service-discovery.md`'s `ArcSwap` pattern) rather than mutating
...
`07-security/ratelimit.md` which degrades far more gracefully against
shared addresses.
...
XDP at the driver (`16-kernel/xdp.md`), where a packet is dropped before
...
feedback loop `07-security/ddos.md` and `labs/17-ebpf` build.
...
7. (Stretch) Wire PROXY protocol v2 parsing (`01-network/proxy-protocol.md`)

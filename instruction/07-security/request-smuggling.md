...
pools and reuses upstream connections (`06-proxy/upstream.md`). The
...
- **Bypassing front-end security entirely.** The proxy enforces auth
  (`auth.md`), IP filtering, and WAF rules on requests it can *see*. A
...
- **Cache poisoning.** Combined with a cache (`05-http-stack/cache.md`),
...
HTTP/2 and speaks HTTP/1.1 upstream (`01-network/http2.md`). HTTP/2 frames

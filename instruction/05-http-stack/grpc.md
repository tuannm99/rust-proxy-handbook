...
## Practice
1. In `labs/05-reverse-proxy`, confirm HTTP/2 trailers are forwarded end-to-end when proxying to an upstream — set up a real gRPC server (e.g. with `tonic`) that returns a non-OK `grpc-status`, and verify the client sees it through the proxy, not a false "success."

...
compression (`05-http-stack/compression.md`) to a gRPC body corrupts it —
...
(`06-proxy/retry.md`), rate limited (`07-security/ratelimit.md`) — the
...
(`05-http-stack/websocket.md`). A server-streaming RPC that emits an
...
Gotcha: body buffering for WAF inspection (`07-security/waf.md`) or retry
replay (`06-proxy/retry.md`) is fundamentally incompatible with streaming
...
(`06-proxy/service-discovery.md`) receive nothing — the connection
lifetime caps from `05-http-stack/keepalive.md` are what eventually
...
`06-proxy/healthcheck.md` needs a gRPC-aware variant. `Watch` is the
...
treating each as a unique URL (`08-observability/metrics.md`).

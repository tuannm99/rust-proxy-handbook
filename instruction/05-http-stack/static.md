...
(`16-kernel/page-cache.md`). In an async runtime, a blocking `sendfile` on
...
(`07-security/ddos.md`), so a cold-filesystem stall can't consume the
whole process.
...
Gotcha: the same normalization discussion as `05-http-stack/router.md`

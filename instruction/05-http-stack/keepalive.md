...
See `05-http-stack/hop-by-hop-headers.md` — including the part where
...
connection is safe are covered in depth in `06-proxy/upstream.md` — this
...
`07-security/request-smuggling.md`. When in doubt, close it; a discarded
...
(`05-http-stack/websocket.md`), a server-streaming gRPC call
(`05-http-stack/grpc.md`), an SSE stream. Applying a 60-second
...
- **It never rebalances.** Scale the upstream pool out
  (`06-proxy/service-discovery.md`) and existing long-lived connections
...
- **Per-connection state accumulates.** Buffers grow to the largest
  ...
  (`14-memory/fragmentation.md`). Recycling connections periodically
  bounds it.
...
it safely (`01-network/http2.md`); this is also the mechanism graceful
shutdown depends on (`09-architecture/graceful-shutdown.md`).
...
retry is conventionally treated as safe — see `06-proxy/upstream.md` for
...
2. Work through `05-http-stack/hop-by-hop-headers.md`'s exercises. **Done
...
8. Measure reuse's effect: compare pooled vs one-connection-per-request
   under `12-testing/load-testing.md`. **Done when** you have p50/p99

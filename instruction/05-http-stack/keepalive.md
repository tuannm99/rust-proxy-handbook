...
This is the part of keep-alive that is specifically a *proxy's* problem,
and getting it wrong is both a correctness bug and a security one.

Some headers describe the *connection between two adjacent hops*, not the
end-to-end message: `Connection`, `Keep-Alive`, `Transfer-Encoding`, `TE`,
`Trailer`, `Upgrade`, `Proxy-Authorization`, `Proxy-Authenticate`. They
are meaningful only on the link they arrived on, and forwarding them to
the next hop is wrong by definition — your connection to the upstream has
its own framing, its own keep-alive state, its own upgrade status.

`Connection` is doubly special: it also *names* other headers to treat as
hop-by-hop. A request carrying `Connection: X-Foo, Keep-Alive` is saying
"`X-Foo` is for you only" — so you must remove `X-Foo` too, then remove
`Connection` itself.
...
```
Gotcha: forwarding `Transfer-Encoding` untouched is one of the standard
ways to *build* a request-smuggling vulnerability
(`07-security/request-smuggling.md`) — the framing for your upstream
connection must be generated by you, from the body you are actually about
to send.

Gotcha: `Connection`-named stripping is also an attack surface in the
other direction. An attacker sending `Connection: Authorization` can
convince a naive proxy to strip a header a downstream component depended
on — or, in the reverse arrangement, strip a security header your own
infrastructure adds. Strip hop-by-hop headers, but apply your own
identity/trust headers *after* stripping (`07-security/auth.md`), never
before.
...
2. Add hop-by-hop header stripping, including `Connection`-named headers.
   **Done when** a request with `Connection: X-Secret` and an `X-Secret`
   header reaches the upstream with neither, and `Transfer-Encoding` is
   regenerated by you rather than forwarded.

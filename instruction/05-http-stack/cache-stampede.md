...
makes things worse. `05-http-stack/cache.md` covers HTTP caching
...
(`09-architecture/rolling-restart.md` — every restart empties an in-memory
...
deadline (`06-proxy/upstream.md`'s request timeouts) and decide for itself
...
`Drop` guard, the same discipline as `06-proxy/upstream.md`'s connection
counter.
...
`stale-while-revalidate` (`05-http-stack/cache.md`) the waiting disappears
...
connection (`06-proxy/upstream.md`), all of them futile.
...
synchronization problem as probe intervals (`06-proxy/healthcheck.md`) and
retry storms (`06-proxy/retry.md`), with the same fix.

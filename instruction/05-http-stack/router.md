...
parser-differential family as `07-security/request-smuggling.md`, applied
...
- **Double-encoding**: `%252e%252e%252f` — decode to a fixed, documented
  depth, matching what your upstream does (`07-security/waf.md` has the
  same discussion).
...
(`09-architecture/config.md`) — beats silently picking one and leaving an
...
- **`OPTIONS`** is a CORS preflight, and browsers send it *without*
  ...
  (`07-security/auth.md`) to preflights, every cross-origin call from a
...
`05-http-stack/vhost-routing.md`), arbitrary headers (canary routing by
`X-Version`, `09-architecture/canary-deploy.md`), sometimes weights for
traffic splitting.
...
there's also the SNI name from the handshake (`01-network/tls.md`), which
...
the pipeline, and `09-architecture/components.md` is where it's designed.
...
runtime (`09-architecture/config.md`, `labs/13-hot-reload`) while requests
are in flight. Build the table as an immutable structure swapped atomically
(`arc_swap`, as in `06-proxy/service-discovery.md`) rather than one mutated

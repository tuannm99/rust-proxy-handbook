...
boundary exactly at your SLO threshold (`08-observability/alerting.md`):
...
(`07-security/waf.md`), connection acquisition from the pool. Without that
...
- **Connection pool saturation** per upstream (`06-proxy/upstream.md`) —
  waiting for a pooled connection is invisible in upstream response time.
- **Retry and circuit-breaker state** (`06-proxy/retry.md`) — retry rate,
  budget exhaustion, circuit transitions.
- **Healthy upstream count** as a gauge (`06-proxy/healthcheck.md`).
- **Cache hit ratio** (`05-http-stack/cache.md`), which explains upstream
  load changes that have nothing to do with client traffic.
- **Queue depth / shed count** (`07-security/ddos.md`), the earliest
  signal of overload.
...
(`17-performance/false-sharing.md`), and at high request rates that shows
up in a profile.
...
which without a flamegraph (`08-observability/profiling.md`).
...
consume proxy CPU (`07-security/ddos.md`). It also means metrics stay
...
7. Pre-resolve label handles on the hot path and profile
   (`08-observability/profiling.md`). **Done when** you can state metrics'

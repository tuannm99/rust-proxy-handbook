# Books
- TCP/IP Illustrated, Vol. 1 (Stevens) — the reference for everything in `01-network/`; read the TCP handshake/retransmit chapters before `01-network/02-tcp.md`.
- The Linux Programming Interface (Kerrisk) — syscall-level detail behind `02-linux/`; read the epoll and signals chapters before `02-linux/01-epoll.md` and `02-linux/04-signals.md`.

...
vulnerable to fragmentation (`14-memory/fragmentation.md`). A slab
...
swapping the global allocator (`02-linux/memory.md`) gets you most of the
...
I/O buffers (dedicated buffer pooling is planned in `14-memory/README.md`).
...
(`08-observability/metrics.md`) so the cap is tuned from data.

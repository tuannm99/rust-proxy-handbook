...
`14-memory/03-object-pool.md` covers pooling same-shaped objects generally.
...
(`14-memory/01-allocator.md`): a handful of fixed tiers (e.g. 4 KB, 16 KB,
...
`02-linux/05-zerocopy.md`'s `sendfile`/`splice` move data from the page
...
for you), WAF body inspection (`07-security/06-waf.md`, which must read the
...
(measure it — `08-observability/02-metrics.md` — rather than guessing), the
...
4. Identify which of `proxy`'s I/O paths are true zero-copy
   (`02-linux/05-zerocopy.md`) and confirm pooling has no effect on those,

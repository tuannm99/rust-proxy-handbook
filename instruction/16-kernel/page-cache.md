...
(`05-http-stack/static.md`) fast, and what makes your proxy's memory
...
`sendfile()` and `splice()` (see `02-linux/zerocopy.md`) move data from the
...
(`03-rust/async.md`'s cooperative-scheduling problem), stalling every other
connection on it.
...
A proxy writing access logs to disk (`08-observability/logging.md`) is a

...
`02-linux/07-epoll.md` covers using epoll from the application side,
...
hands off to a softirq (`16-kernel/04-interrupt.md`) that walks the network
...
behind the missed-wakeup bug `02-linux/07-epoll.md`'s exercise has you
...
2. Reproduce the edge-triggered missed-wakeup bug from `02-linux/07-epoll.md`

...
Deeper than `02-linux/01-epoll.md`/`02-io_uring.md`'s application-facing
treatment — what's actually happening inside the kernel below the syscall
you called.

## Status: written; read `02-linux/01-epoll.md`/`02-io_uring.md` first

Everything below assumes you've already hit the application-facing
version of the same mechanism (`02-linux/01-epoll.md`'s `EAGAIN` bug,
`02-linux/02-io_uring.md`'s basic submit/complete usage) and want to know why
...
- `08-page-cache.md` — how the kernel caches file data, and how that interacts with `mmap`/`sendfile` in `02-linux/05-zerocopy.md`

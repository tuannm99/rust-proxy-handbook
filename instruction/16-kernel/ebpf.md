...
(`16-kernel/xdp.md`) and for most modern kernel-level observability.
...
`07-security/ip-filtering.md`), and `RINGBUF` (efficient
...
- **XDP** — earliest possible, in the NIC driver before an `sk_buff` exists.
  Fastest, most restricted. See `16-kernel/xdp.md`.
...
anything (`07-security/ddos.md`), or observe the kernel without
...
4. Build an `LPM_TRIE` map of blocked CIDRs, populate it from user space,
   ...
   matching `07-security/ip-filtering.md` does in the proxy.

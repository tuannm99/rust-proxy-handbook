...
`01-network/tcp.md` covers TCP from the application's side (the
...
- **SYN backlog** (half-open connections, in `SYN_RCVD`, waiting for the
  ...
  SYN flood exhausts (`07-security/ddos.md`).
...
RSS accounting (`02-linux/memory.md`). Watch `/proc/net/sockstat` and
...
connections (`06-proxy/upstream.md`'s pooling, which avoids opening and
...
1. Watch `ss -tan state time-wait | wc -l` while driving load at
   ...
   request; then add connection pooling (`06-proxy/upstream.md`) and
   compare the count.

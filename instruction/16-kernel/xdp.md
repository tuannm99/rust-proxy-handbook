...
eXpress Data Path: run an eBPF program (`16-kernel/ebpf.md`) in the NIC
...
That is the entire value proposition for `07-security/ddos.md`: under a
...
accept-rate limiting and HTTP-layer defenses (`07-security/ratelimit.md`,
`07-security/waf.md`). Blocklists come *from* the proxy — which sees the
...
6. Close the loop: have `proxy` detect an abusive source via
   `07-security/ratelimit.md` and push it into the XDP map with a TTL;

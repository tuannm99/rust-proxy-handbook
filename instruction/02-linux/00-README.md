...
- `04-memory-basics.md` — virtual address space (a pointer into `08-memory.md`'s depth), the cache/RAM/disk hierarchy, stack vs heap
...
**Kernel mechanisms:**
- `06-epoll.md` — readiness notification, level- vs edge-triggered, the `EAGAIN` bug you should hit yourself
- `07-io_uring.md` — the newer async I/O interface and where it differs in kind from epoll
- `08-memory.md` — virtual memory, paging, RSS, swapping out the global allocator
- `09-signals.md` — signal handling in an async process, `SIGTERM`/`SIGHUP` conventions
- `10-zerocopy.md` — `sendfile`, `splice`, `mmap`, and when they actually pay

## Where it goes next

The fundamentals group first if you need it — everything else assumes it.
`06-epoll.md` underpins `04-runtime/01-tokio.md`; `09-signals.md`
underpins `09-architecture/03-config.md` and `04-graceful-shutdown.md`;
`10-zerocopy.md` underpins `05-http-stack/05-static.md`. For what happens

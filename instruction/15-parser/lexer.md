...
stage before any structure exists. `05-http-stack/parser.md` does this
inline for HTTP; this file is the reusable theory underneath, equally
applicable to the config parser in `09-architecture/config.md`.
...
same "bytes, not String" discipline as `05-http-stack/parser.md`.
...
so simple that `05-http-stack/parser.md` scans bytes directly with no token
...
4. Benchmark borrowed-range tokens against a version that allocates a
   ...
   difference.
5. Compare your hand-written scanner against `05-http-stack/parser.md`'s

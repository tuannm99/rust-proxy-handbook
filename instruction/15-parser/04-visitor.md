...
across your codebase. Follows `15-parser/03-ast.md`.

## What to learn

### The problem the visitor pattern solves
Once you have a tree (`15-parser/03-ast.md`), you will walk it more than once:
...
- **`&mut self` walk** for transformation — constant folding, applying
  ...
  usual escape is the arena form from `15-parser/03-ast.md` — walk indices and
...
same judgement call as "do I even need an AST" in `15-parser/03-ast.md`.

### Where a proxy actually meets this
Config processing is the honest use case: parse (`15-parser/02-parser.md`) →
AST (`15-parser/03-ast.md`) → a validation visitor (every `upstream`
...
(`09-architecture/03-config.md`). Keeping validation and lowering as separate
visitors means an invalid config is rejected *whole*, before any part of
the new config goes live — which is the atomicity `09-architecture/03-config.md`
requires.

## Practice
1. Add a read-only validation visitor over the config AST from
   `15-parser/03-ast.md` that checks cross-references (every `route`'s

...
- **Certificate and key actually load and match** (`01-network/tls.md`) —
  a path typo or a mismatched pair is a total outage for that vhost.
- **Routes don't conflict** (`05-http-stack/router.md`) — two rules that
...
`06-proxy/service-discovery.md` in a new costume. Validate syntax and
...
actively misled. Either apply it (rebind, which `09-architecture/rolling-restart.md`
...
(`09-architecture/components.md`'s extensions). This bug is rare, entirely
...
refcount-driven drain from `06-proxy/service-discovery.md`. A long-lived
...
newtype (`08-observability/logging.md`) so `{:?}` on the config can't leak
them.
...
alert on reload *failures* as a ticket (`08-observability/alerting.md`):

...
(`05-http-stack/static.md`'s immutable assets), and the user sees a broken
...
- **Minimum sample count** before any comparison is evaluated — the same
  floor as the alerting ratio in `08-observability/alerting.md` and the
  rate-based circuit breaker in `06-proxy/retry.md`. Below it, the correct
...
- **Resource leaks.** A memory leak (`14-memory/fragmentation.md`) or fd
...
- **Load-dependent failures.** Lock contention, connection pool exhaustion
  (`06-proxy/upstream.md`), and thundering herds only appear near capacity
  — which a 1% canary is nowhere near.
- **Time-dependent bugs.** A daily batch, a certificate expiry
  (`01-network/tls.md`), a month-boundary calculation.
...
panic-threshold and health-check logic from `06-proxy/healthcheck.md`
...
1. Extend `labs/06-load-balancer` (or `proxy`) to support two named pools
   ...
   (`09-architecture/config.md`). **Done when** weights can change without
   a restart.
...
6. Simulate a bad canary under load (`12-testing/load-testing.md`) with

...
(`06-proxy/service-discovery.md`). If the proxy registers itself, active
...
`05-http-stack/keepalive.md`'s close/request collision, now happening
...
(`05-http-stack/grpc.md`), or an SSE stream may be minutes or hours from
...
- **Log and trace buffers** — `tracing_appender`'s non-blocking writer and
  the OTLP batch exporter (`08-observability/tracing.md`) both hold
...
- **Upstream connections** — close pooled idle connections explicitly
  (`06-proxy/upstream.md`) rather than letting the process exit drop them,
...
4. Load-test while sending `SIGTERM` mid-test
   (`12-testing/load-testing.md`). **Done when** in-flight requests see
...
9. If you built service discovery (`06-proxy/service-discovery.md`),

...
reconnects with backoff (`retry.md`), re-lists the full state on
...
(`healthcheck.md`), applied at the membership layer instead.
...
3. **Close its pooled idle connections** (`upstream.md`) last. Skipping
...
Gotcha: a graceful-shutdown path (`09-architecture/graceful-shutdown.md`)

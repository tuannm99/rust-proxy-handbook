...
it never executes (`03-rust/async.md`: drop *is* cancel). Every cancelled
request permanently inflates the count. Least-connection balancing
(`load-balancer.md`) then routes *away* from a perfectly healthy upstream
...
(`14-memory/object-pool.md`) — any manual "increment, do work, decrement"
...
(`01-network/tls.md`), paid before a single request byte moves. Keep a
...
(`01-network/http2.md`), past which new streams queue behind finished ones
...
(`16-kernel/tcp-stack.md`). The upstream's own accept backlog, not your
...
deliberately, and see `retry.md` for the general idempotency rule this is
an exception to.

Gotcha: the retry-once-on-fresh-connection path must not consume the
retry budget from `retry.md` — it's a connection-liveness retry, not a

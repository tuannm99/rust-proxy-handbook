...
trust inside a private network — tie to `07-security/01-auth.md` for how this
...
as a config-reload concern, see `09-architecture/03-config.md`.
...
5. Simulate a cert rotation (swap the cert file, trigger reload per
   `09-architecture/03-config.md`) and confirm existing connections aren't
   dropped mid-request.

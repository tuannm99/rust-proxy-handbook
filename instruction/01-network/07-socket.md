...
(see `02-linux/07-epoll.md`'s exercise) comprehensible instead of magic.
...
(epoll, see `02-linux/07-epoll.md`) is built on: register the fd, block on
...
2. Implement `labs/00-tcp-server` using tokio's `TcpListener`, then
   ...
   `02-linux/07-epoll.md`'s exercise (a scratch project, not part of this

...
(`05-http-stack/07-cache.md`, `13-algorithms/lru.md`) is not enough. The one
...
(often over gossip, `18-distributed/02-gossip.md`) accepting brief
inconsistency. A proxy cache almost always chooses eventual consistency
here — strong consistency (`18-distributed/01-raft.md`) on a data-path cache
...
single-node version (`05-http-stack/07-cache.md`'s request coalescing). The
...
5. Implement purge with versioned keys and a gossiped invalidation
   (`18-distributed/02-gossip.md`); reason explicitly about the staleness
   window and why eventual consistency is the right call for a proxy cache
   versus `18-distributed/01-raft.md`.

...
- Hyper — Rust. The HTTP library this handbook's `labs/` and `proxy/` crates are built on; read its `h1`/`h2` codec source alongside `05-http-stack/parser.md`.
- Tokio — Rust. The async runtime underneath everything from `04-runtime/tokio.md` onward; read the reactor/scheduler source after doing the hand-rolled-executor exercise in `03-rust/async.md`.

...
## Practice
1. In the hand-rolled executor from `03-rust/05-async.md`'s exercise, implement a `Waker` via `std::task::Wake`: `wake()` should push the task id back onto a run queue (e.g. a `VecDeque` behind a `Mutex`, or an `mpsc` channel).

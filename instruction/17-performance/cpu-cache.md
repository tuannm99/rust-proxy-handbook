...
Read this **after** `08-observability/profiling.md` has pointed a
...
- **Spatial** — data used together should sit together, so one line fetch
  ...
  `13-algorithms/lru.md` and `15-parser/ast.md` push arenas (`Vec` +
...
(`06-proxy/load-balancer.md`), the struct-of-arrays layout can be several
...
(`08-observability/profiling.md`) showing a specific hot loop, under real
load (`12-testing/load-testing.md`) — see also `17-performance/false-sharing.md`
for the concurrent version of this problem.

## Practice
1. Benchmark array-of-structs vs struct-of-arrays for scanning one field
   across 100k backends (the `06-proxy/load-balancer.md` weight scan);

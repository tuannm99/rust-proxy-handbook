...
complexity. The end of the road that `17-performance/branch-prediction.md`
started down.
...
`17-performance/branch-prediction.md` worried about.
...
- **Delimiter search** in the HTTP parser (`05-http-stack/parser.md`):
...
1. **Use a crate that already did it.** `memchr` (delimiter search),
   ...
   SIMD internally for the WAF scan in `07-security/waf.md`). This covers
...
3. **Portable SIMD / intrinsics.** `std::simd` (portable) or
   ...
   are `unsafe` (`03-rust/unsafe.md`) and CPU-specific.
...
only after `08-observability/profiling.md` shows a byte-scanning loop is a
top hot spot under real load (`12-testing/load-testing.md`), and even then
...
1. Replace a hand-written byte-search loop in your HTTP parser
   (`05-http-stack/parser.md`) with `memchr` and benchmark the header-scan
...
4. Confirm `aho-corasick` (`13-algorithms/aho-corasick.md`) is using SIMD
   under your WAF workload (`07-security/waf.md`) and compare its throughput

...
Desugaring an `async fn` into a state-machine struct (see `03-rust/05-async.md`)
...
2. In the hand-rolled executor from `03-rust/05-async.md`'s exercise, define your `Task` type as
...
3. Explain in your own words (a comment is fine) why `Future::poll` takes
   ...
   the self-referential struct shape from `03-rust/05-async.md`.
4. Read the standard library docs for `Pin::get_mut` and `Pin::new_unchecked`
   ...
   uphold manually (this is unsafe — see `03-rust/03-unsafe.md`).
